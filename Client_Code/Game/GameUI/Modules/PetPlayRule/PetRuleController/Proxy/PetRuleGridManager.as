package GameUI.Modules.PetPlayRule.PetRuleController.Proxy
{
	...
	public class PetRuleGridManager extends Proxy
	{
		public static const NAME:String = "petRuleGridManager";
		...
		public function PetRuleGridManager(list:Array, gridSprite:MovieClip)
		{
			super(NAME);
			this.gridList = list;
...
//				gridUint.Grid.addEventListener(MouseEvent.DOUBLE_CLICK, doubleClickHandler);
		...
		public function showItems(list:Array):void
		{
			...
			{
				for(var tag:int = 0; tag < list.length; tag++) 
				{
					PetSkillLearnConstData.GridUnitList[tag].HasBag = true;
					if(list[tag] == undefined) {
						continue;
					}
					var useItem:SkillItem = new SkillItem(list[tag].gameSkill.SkillID, gridSprite);		//new UseItem(list[j].index, list[j].type, gridSprite);
					useItem.Num = 1;
					useItem.x = PetSkillLearnConstData.GridUnitList[tag].Grid.x + 2;
					useItem.y = PetSkillLearnConstData.GridUnitList[tag].Grid.y + 2;
					
					useItem.Id = list[tag].gameSkill.SkillID;			//技能ID
	//				useItem.IsBind = list[j].isBind;
					useItem.Type = list[tag].Level;					//技能等级		
					
					useItem.IsLock = false;
					PetSkillLearnConstData.GridUnitList[tag].Item = useItem;
					PetSkillLearnConstData.GridUnitList[tag].IsUsed = true;
