...
//									Math.abs(p_endY - note[1]) * this.COST_VERTICAL;		// 这句会认人走得不自然		
		...
		public function floyd(path:Array):Array {
			if (path == null)
				return null;
			var floydPath:Array = new Array();
			var pointItem:Array = (path[0] as Array).concat();
			pointItem.push(0);
				
			floydPath.push(pointItem);
			var chkTmp:Array = (path[0]).concat();
			var size:int = 0;
			for (var i:int = 1; i < (path.length-1); i++) 
			{
				var _startPoint:Point = MapTileModel.GetTilePointToStage(chkTmp[0], chkTmp[1]);
				var _endPoint:Point = MapTileModel.GetTilePointToStage(path[i][0], path[i][1]);
				size++;
				if(!m_mapTileModel.IsPassAToB(_startPoint.x,_startPoint.y, _endPoint.x, _endPoint.y,5)){
					pointItem = (path[i] as Array).concat();
					pointItem.push(size);
					floydPath.push(pointItem);
					chkTmp = pointItem;
					size = 0;
			...
			size++;
			pointItem = (path[path.length-1] as Array).concat();
			pointItem.push(size);
			floydPath.push(pointItem);
